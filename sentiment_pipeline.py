...
cur.execute("""CREATE TABLE IF NOT EXISTS sentiment_raw_factors (
    ...
    composite_index DECIMAL(10,4),
    sector_up INT,
    sector_down INT,
    sector_ad_ratio DECIMAL(6,4),
    limit_up INT, limit_down INT,
    turnover DECIMAL(15,2),
    main_force_net DECIMAL(15,2),
    volume_pctile_60d DECIMAL(5,2),
    margin_balance DECIMAL(20,2),
    sentiment_label DECIMAL(5,2),
...
def fetch_all():
    ...
    try:
        ...
        sz_margin = ak.stock_margin_szse(date=TODAY.replace('-',''))
        if len(sh_margin) > 0:
            sh_col = sh_margin.columns[-1]
