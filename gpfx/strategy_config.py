# 策略版本管理
# 每次修改参数后，在底部追加一条记录，不要修改历史记录
# 运行 _afternoon_check.py 或 simulate.py 时自动读取最新版本参数

VERSIONS = [
    {
        "ver": "v5.4",
        "active": "2026-05-20~2026-05-31",
        "desc": "单指数情绪+买A抄底+卖一半+止盈+止损",
        "params": {
            "buyA_sv_max": -0.8,
            "buyA_dc_min": -1.0,
            "buyB": None,
            "sell_half_sv": 1.5,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "take_profit_pct": 2.0,
            "max_invest": 10000,
            "composite_wt": [1.0],
        },
    },
    {
        "ver": "v5.5a",
        "active": "2026-06-01~",
        "desc": "复合指数+买A抄底+买B趋势全仓+卖一半1.8+止盈",
        "params": {
            "buyA_sv_max": -0.8,
            "buyA_dc_min": -1.0,
            "buyB": {"sv_min": -0.5, "sv_max": 0.5, "position": 0.5},
            "sell_half_sv": 1.8,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "take_profit_pct": 2.0,
            "max_invest": 10000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
        },
    },
    {
        "ver": "v5.5b",
        "active": "2026-06-01~",
        "desc": "复合指数+买A抄底+买B趋势全仓+卖一半1.8+去止盈",
        "params": {
            "buyA_sv_max": -0.8,
            "buyA_dc_min": -1.0,
            "buyB": {"sv_min": -0.5, "sv_max": 0.5, "position": 1.0},
            "sell_half_sv": 1.8,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "take_profit_pct": None,
            "max_invest": 10000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
        },
    },
    {
        "ver": "v5.5c",
        "active": "2026-06-01~",
        "desc": "复合指数+买A≤-0.5+买B全仓+去止盈+去卖一半+8000仓",
        "params": {
            "buyA_sv_max": -0.5,
            "buyA_dc_min": -1.0,
            "buyB": {"sv_min": -0.5, "sv_max": 0.5, "position": 1.0},
            "sell_half_sv": None,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "take_profit_pct": None,
            "max_invest": 8000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
        },
    },
    {
        "ver": "v5.5d",
        "active": "2026-06-01~",
        "desc": "复合指数+买A≤-1.2+买B全仓+去止盈+去卖一半+8000仓(当前最佳)",
        "params": {
            "buyA_sv_max": -1.2,
            "buyA_dc_min": -1.0,
            "buyB": {"sv_min": -0.5, "sv_max": 0.5, "position": 1.0},
            "sell_half_sv": None,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": None,
            "take_profit_pct": None,
            "max_invest": 8000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
        },
    },
    {
        "ver": "v5.5e",
        "active": "2026-06-01~",
        "desc": "v5.5d + 回撤止损(从峰值-3%)",
        "params": {
            "buyA_sv_max": -1.2,
            "buyA_dc_min": -1.0,
            "buyB": {"sv_min": -0.5, "sv_max": 0.5, "position": 1.0},
            "sell_half_sv": None,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": -3.0,
            "take_profit_pct": None,
            "max_invest": 8000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
        },
    },
    {
        "ver": "v5.6",
        "active": "2026-06-01~",
        "desc": "v5.5d + 仓位升级1万 (回测+10.06%/-2.49%)",
        "params": {
            "buyA_sv_max": -1.2,
            "buyA_dc_min": -1.0,
            "buyB": {"sv_min": -0.5, "sv_max": 0.5, "position": 1.0},
            "sell_half_sv": None,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": None,
            "take_profit_pct": None,
            "max_invest": 10000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
        },
    },
    {
        "ver": "v5.7",
        "active": "2026-06-01~",
        "desc": "v5.6 + 买B情绪动量增强(3日情绪均线↑ + 动态仓位 + 乖离)",
        "params": {
            "buyA_sv_max": -1.2,
            "buyA_dc_min": -1.0,
            "buyB": {
                "sv_min": -0.5, "sv_max": 0.5,
                "position_max": 1.0,
                "position_min": 0.5,
                "sent_ma_days": 3,
                "ma_deviation_max": 0.03,
            },
            "sell_half_sv": None,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": None,
            "take_profit_pct": None,
            "max_invest": 10000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
        },
    },
    {
        "ver": "v5.8",
        "active": "2026-06-03~",
        "desc": "v5.6 + margin_boost(融资去杠杆>1%时买A阈值从-1.2放宽到-1.0)",
        "params": {
            "buyA_sv_max": -1.2,
            "buyA_dc_min": -1.0,
            "buyA_margin_boost": True,
            "buyB": {"sv_min": -0.5, "sv_max": 0.5, "position": 1.0},
            "sell_half_sv": None,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": None,
            "take_profit_pct": None,
            "max_invest": 10000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
        },
    },
    {
        "ver": "v6.0",
        "active": "2026-07-02~",
        "desc": "K-means聚类情绪(6因子z-score标准化)+v5.6参数, 去等频分箱循环论证",
        "params": {
            "buyA_sv_max": -1.2,
            "buyA_dc_min": -1.0,
            "buyA_margin_boost": True,
            "buyB": {"sv_min": -0.5, "sv_max": 0.5, "position": 1.0},
            "sell_half_sv": None,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": None,
            "take_profit_pct": None,
            "max_invest": 10000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
            "sentiment_formula": "sentiment = -0.2917 + 0.3861 × composite",
            "sentiment_source": "sentiment_v6",
        },
    },
    {
        "ver": "v6.1",
        "active": "2026-07-02~",
        "desc": "v6.0 + 仓位提升至15000 (网格搜索最优, 收益+52% vs v6.0, 回撤+43%)",
        "params": {
            "buyA_sv_max": -1.2,
            "buyA_dc_min": -1.0,
            "buyA_margin_boost": True,
            "buyB": {"sv_min": -0.5, "sv_max": 0.5, "position": 1.0},
            "sell_half_sv": None,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": None,
            "take_profit_pct": None,
            "max_invest": 15000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
            "sentiment_formula": "sentiment = -0.2917 + 0.3861 × composite",
            "sentiment_source": "sentiment_v6",
        },
    },
    {
        "ver": "v6.2",
        "active": "2026-07-02~",
        "desc": "v6.1 + 3档金字塔分批买入(-1.2买50%, -1.8买30%, -2.5买20%), 全仓20000, 回撤更低",
        "params": {
            "buyA_sv_max": -1.2,
            "buyA_dc_min": -1.0,
            "buyA_margin_boost": True,
            "buyA_batch": [
                {"sv_max": -1.2, "position": 0.5},
                {"sv_max": -1.8, "position": 0.3},
                {"sv_max": -2.5, "position": 0.2},
            ],
            "buyB": {"sv_min": -0.5, "sv_max": 0.5, "position": 1.0},
            "sell_half_sv": None,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": None,
            "take_profit_pct": None,
            "max_invest": 20000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
            "sentiment_formula": "sentiment = -0.2917 + 0.3861 × composite",
            "sentiment_source": "sentiment_v6",
        },
    },
    {
        "ver": "v6.3",
        "active": "2026-07-02~",
        "desc": "v6.1 + 回撤-7%止盈: 从峰值回落7%清仓, 收益+18.28%回撤6.86% (回撤降35% vs v6.1)",
        "params": {
            "buyA_sv_max": -1.2,
            "buyA_dc_min": -1.0,
            "buyA_margin_boost": True,
            "buyB": {"sv_min": -0.5, "sv_max": 0.5, "position": 1.0},
            "sell_half_sv": None,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": -7.0,
            "take_profit_pct": None,
            "max_invest": 15000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
            "sentiment_formula": "sentiment = -0.2917 + 0.3861 × composite",
            "sentiment_source": "sentiment_v6",
        },
    },
    {
        "ver": "v6.4",
        "active": "2026-07-02~",
        "desc": "v6.3 + 盈利锁定: 盈利≥8%后回撤止损从-7%收紧到-3%, 收益+21.66%回撤5.97% (R/R=3.6)",
        "params": {
            "buyA_sv_max": -1.2,
            "buyA_dc_min": -1.0,
            "buyA_margin_boost": True,
            "buyB": {"sv_min": -0.5, "sv_max": 0.5, "position": 1.0},
            "sell_half_sv": None,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": -7.0,
            "profit_lock": 8.0,
            "lock_trailing_stop": -3.0,
            "take_profit_pct": None,
            "max_invest": 15000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
            "sentiment_formula": "sentiment = -0.2917 + 0.3861 × composite",
            "sentiment_source": "sentiment_v6",
        },
    },
    {
        "ver": "v6.5",
        "active": "2026-07-02~",
        "desc": "v6.4 + 中国神华避险: MA5>MA5减仓至50%, ≤MA5加仓至150%, 收益+27.35%回撤4.70% (R/R=5.8)",
        "params": {
            "buyA_sv_max": -1.2,
            "buyA_dc_min": -1.0,
            "buyA_margin_boost": True,
            "buyB": {"sv_min": -0.5, "sv_max": 0.5, "position": 1.0},
            "sell_half_sv": None,
            "sell_all_sv": 2.0,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": -7.0,
            "profit_lock": 8.0,
            "lock_trailing_stop": -3.0,
            "take_profit_pct": None,
            "max_invest": 15000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
            "sentiment_formula": "sentiment = -0.2917 + 0.3861 × composite",
            "sentiment_source": "sentiment_v6",
            "haven_code": "601088",
            "haven_name": "中国神华",
            "haven_ma_window": 5,
            "haven_scale": 0.5,
            "risk_scale": 1.5,
        },
    },
{
        "ver": "v6.6",
        "active": "2026-07-23~",
        "desc": "510300专用: 阈值放宽(买A≤-0.8/跌≥-0.5%), 卖出1.5, 回撤-5%, 盈利锁5%, 去神华避险",
        "params": {
            "buyA_sv_max": -0.8,
            "buyA_dc_min": -0.5,
            "buyA_margin_boost": True,
            "buyB": {"sv_min": -0.3, "sv_max": 0.3, "position": 1.0},
            "sell_half_sv": 1.0,
            "sell_all_sv": 1.5,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": -5.0,
            "profit_lock": 5.0,
            "lock_trailing_stop": -2.5,
            "take_profit_pct": None,
            "max_invest": 15000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
            "sentiment_formula": "sentiment = -0.2917 + 0.3861 × composite",
            "sentiment_source": "sentiment_v6",
        },
    },
    {
        "ver": "v6.6a",
        "active": "2026-07-23~",
        "desc": "510300专用: v6.6 + 神华MA5避险保留(避险50%/风险150%)",
        "params": {
            "buyA_sv_max": -0.8,
            "buyA_dc_min": -0.5,
            "buyA_margin_boost": True,
            "buyB": {"sv_min": -0.3, "sv_max": 0.3, "position": 1.0},
            "sell_half_sv": 1.0,
            "sell_all_sv": 1.5,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": -5.0,
            "profit_lock": 5.0,
            "lock_trailing_stop": -2.5,
            "take_profit_pct": None,
            "max_invest": 15000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
            "sentiment_formula": "sentiment = -0.2917 + 0.3861 × composite",
            "sentiment_source": "sentiment_v6",
            "haven_code": "601088",
            "haven_name": "中国神华",
            "haven_ma_window": 5,
            "haven_scale": 0.5,
            "risk_scale": 1.5,
        },
    },
    {
        "ver": "v6.7",
        "active": "2026-07-24~",
        "desc": "510300专用: 新情绪尺度网格搜索优化(买A≤-0.4/跌≥-0.3%), 卖半0.3/清仓0.6, 收益+11.80%回撤4.66%",
        "params": {
            "buyA_sv_max": -0.4,
            "buyA_dc_min": -0.3,
            "buyA_margin_boost": True,
            "buyB": {"sv_min": -0.2, "sv_max": 0.2, "position": 1.0},
            "sell_half_sv": 0.3,
            "sell_all_sv": 0.6,
            "stop_loss_pct": -3.0,
            "trailing_stop_pct": -5.0,
            "profit_lock": 5.0,
            "lock_trailing_stop": -2.5,
            "take_profit_pct": None,
            "max_invest": 20000,
            "composite_wt": [0.3, 0.2, 0.1, 0.4],
            "sentiment_formula": "sentiment = -0.2917 + 0.3861 × composite",
            "sentiment_source": "sentiment_v6",
            "haven_code": "601088",
            "haven_name": "中国神华",
            "haven_ma_window": 5,
            "haven_scale": 0.5,
            "risk_scale": 1.5,
        },
    },
]

def get_latest():
    return VERSIONS[17]  # v6.7 (新情绪尺度网格搜索优化, 买A≤-0.4/跌≥-0.3%, 2026-07-24)


def find(ver_str):
    for v in VERSIONS:
        if v["ver"] == ver_str:
            return v
    return None


# ── 513530 港股通红利ETF 独立策略 ──
# 均线乖离策略: MA20乖离 -2.5%买入 / +5%卖出，中间持有吃月分红
STRATEGY_513530 = {
    "ver": "h1.0",
    "active": "2026-06-09~",
    "desc": "港股红利513530: MA20乖离-2.5%买入/+5%卖出+持有吃息",
    "fund_code": "513530",
    "fund_name": "港股通红利ETF",
    "params": {
        "ma_fast": 20,
        "buy_deviation": -3.0,
        "sell_deviation": 5.0,
        "stop_loss_pct": -8.0,
        "max_invest": 10000,
        "position_pct": 0.95,
    },
}