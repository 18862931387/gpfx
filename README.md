...
**计算方式**（v3）：基于四指数加权（上证40%+深证30%+创业板20%+科创50 10%），叠加涨跌停比、成交额、主力资金流向修正（详见 `sentiment_calibrate.py`）

**版本演进**：
| 版本 | 输入参数 | 修正项 |
|:----|:---------|:-------|
| v1 | 四指数加权 | 无 |
| v2 | 上证+涨跌停 | 涨跌停比 >3 或 <0.5 时 ±0.3 |
| v3 | 上证+涨跌停+成交额+主力资金流向 | 放量/缩量/主力大额流入流出 |
...
| `sentiment_calibrate.py` | **情绪计算核心库**：v1/v2/v3 三版算法 |
| `_calibrate_sentiment.py` | 情绪校准脚本（拉取指数+写入DB） |
| `_backtest_v53.py` | v5.3 综合版回测（带成交额+跌停过滤） |
...
> 以下基于情绪 v2 算法。v3 算法（引入成交额+主力资金修正）下 3-5月回测收益 **+7.70%**（见 `_backtest_v53.py`）
...
4. **情绪校准**: `market_sentiment` 基于四指数加权（上证40%+深证30%+创业板20%+科创50 10%），经涨跌停比、成交额、主力资金流向三步修正（v3）。`sentiment_calibrate.py` 为核心算法库，`_calibrate_sentiment.py` 为运行脚本
