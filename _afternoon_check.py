...
# 3. 大盘资金流向明细 (push2his日线, 沪深两市合并, 稳定不受限流)
import time
main_force_net = 0.0
...
try:
    ...
    hgt = nb_data.get('hgt',{}).get('klines',[])  # 沪股通
    sgt = nb_data.get('sgt',{}).get('klines',[])  # 深股通
    if hgt and sgt:
...
def calc_sentiment(composite, limit_up, limit_down, turnover, mf_net, margin_chg, northbound_net, news_score, data_is_today):
    # 基础分: 复合指数回归
    # v6 K-means 聚类标定 (2026-07-02): sentiment = -0.1920 + 0.7169 × composite, R²=0.35
    # v5 旧标定 (循环论证): sentiment = 0.3461 + 0.8168 × composite, R²=0.92(虚高)
    val = -0.1920 + 0.7169 * composite  # v6
...
# 近失信号: 3/4满足时记录到 near_miss_signals
near_miss = sum([buy_cond1, buy_cond2, buy_cond3, buy_cond4])
if not buy_signal and near_miss >= 3:
    try:
        import pymysql
        conn_nm = pymysql.connect(**DB)
        cur_nm = conn_nm.cursor()
        cur_nm.execute("""CREATE TABLE IF NOT EXISTS near_miss_signals (
            id INT AUTO_INCREMENT PRIMARY KEY,
            trade_date DATE NOT NULL UNIQUE,
            sentiment_value DECIMAL(5,2), etf_pct DECIMAL(6,2), turnover DECIMAL(10,0),
            cond1_sentiment TINYINT, cond2_etf_drop TINYINT, cond3_volume TINYINT, cond4_main_force TINYINT,
            tail_main_force DECIMAL(12,0), main_force_net DECIMAL(16,0), super_large_net DECIMAL(16,0),
            miss_reason VARCHAR(100), create_time DATETIME DEFAULT NOW(),
            INDEX idx_date (trade_date)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4""")
        tail_mf = (fund_flow[-1]['main_net'] if fund_flow and len(fund_flow) > 0 else 0)
        reasons = []
        if not buy_cond1: reasons.append(f'情绪{SENT:+.1f}未达恐慌')
        if not buy_cond2: reasons.append(f'跌幅{pct:.1f}%未达要求')
        if not buy_cond3: reasons.append(f'成交{tv:.0f}亿不够放量/跌停不够')
        if not buy_cond4: reasons.append(f'尾盘主力流出，缺抄底确认')
        cur_nm.execute("""INSERT INTO near_miss_signals
            (trade_date,sentiment_value,etf_pct,turnover,cond1_sentiment,cond2_etf_drop,cond3_volume,cond4_main_force,tail_main_force,main_force_net,super_large_net,miss_reason,create_time)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW())
            ON DUPLICATE KEY UPDATE sentiment_value=VALUES(sentiment_value),etf_pct=VALUES(etf_pct),turnover=VALUES(turnover),
            cond1_sentiment=VALUES(cond1_sentiment),cond2_etf_drop=VALUES(cond2_etf_drop),cond3_volume=VALUES(cond3_volume),cond4_main_force=VALUES(cond4_main_force),
            tail_main_force=VALUES(tail_main_force),main_force_net=VALUES(main_force_net),super_large_net=VALUES(super_large_net),miss_reason=VALUES(miss_reason)""",
            (TODAY, SENT, pct, tv, buy_cond1, buy_cond2, buy_cond3, buy_cond4, tail_mf, main_force_net, super_large_net, '; '.join(reasons)))
        conn_nm.commit()
        conn_nm.close()
    except: pass
...
# v6.3 trailing stop: 从持仓峰值回落X%清仓
trail_peak = entry
sell_trail = False
trail_limit = None
if P.get("trailing_stop_pct") and shares > 0 and entry > 0:
    try:
        ...
        if r and r[0]:
            ...
            trail_peak = max(trail_peak, cur_p)  # update peak
        else:
...
if shares > 0:
    if sell_stop:
        ...
        print(f'  ❗ 回撤止损{lock_tag}! 从峰值{trail_peak:.4f}回落{trail_pnl:.1f}%≤{trail_limit}%')
        print(f'  → 清仓 {shares:.0f}股 × {cur_p:.4f}')
    elif sell_sent:
    ...
    haven_tag = f' [神华避险减仓]' if haven_above_ma else (' [神华风险加仓]' if haven_above_ma is not None else '')
    print(f'  ✅ 买A抄底!{haven_tag} {bs}股 × {cur_p:.4f} = {bs*cur_p:.0f}元')
    ...
    # v6.5 避险仓位
    if haven_above_ma is not None:
        amt = min(MAX_PER_TRADE * pos * (P["haven_scale"] if haven_above_ma else P["risk_scale"]), cash)
    bs = int(amt / cur_p / 100) * 100
    pf = f'{pos*100:.0f}%'
    extras = ''
    if buyB_enhanced:
...
if sell_stop:
    ...
    signal_type = 'trailing_stop'; signal_reason = f'从峰值{trail_peak:.4f}回落{trail_pnl:.1f}%≤{trail_limit}%'
elif sell_sent:
...
# ── 510300 个股资金流落地 (push2 ulist, v6替代废弃的push2his) ──
try:
    import pymysql
